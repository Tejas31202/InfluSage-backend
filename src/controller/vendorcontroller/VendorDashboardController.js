...
export const getVendorRecentCampaigns = async (req, res) => {
  try {
    const p_userid = req.user?.id || req.body.p_userid;
    if (!p_userid) {
      return res.status(400).json({ message: 'User ID is required to fetch recent campaigns.' });
    }
    const result = await client.query(
    ...
    if (result.rowCount === 0 || !result.rows[0]) {
      return res.status(404).json({ message: 'No recent campaigns found for the given user.' });
    }
