...
export const getVendorPendingContentList = async (req, res) => {
  ...
  if (!p_userid) return res.status(HTTP.BAD_REQUEST).json({ Message: "User If Required For Get Pending Content" });
  try {
...
export const getVendorCampaignStatusOverview = async (req, res) => {
  ...
  if (!p_userid) return res.status(HTTP.BAD_REQUEST).json({ Message: "User Id Required For Get Campaign Status OverView" });
...
export const getVendorDashboardFeedbackList = async (req, res) => {
  const p_userid = req.user?.id || req.query.p_userid
  if (!p_userid) return res.status(HTTP.BAD_REQUEST).json({ Message: "User Id Required For Getting Feedback" })
  try {
...
export const getCampaignStatusTrend = async (req, res) => {
  const p_userid = req.user?.id;
  if (!p_userid) return res.status(HTTP.BAD_REQUEST).json({ message: "User Id Required For Get Campaign Status Trend" });
